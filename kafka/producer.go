...
type (
	...
	AsyncProducer interface {
		...
		ProduceSync(value sarama.Encoder, options ...MsgOption) <-chan *AsyncResult
...
func NewAsyncProducer(address []string, config *sarama.Config, topic string) (AsyncProducer, error) {
	...
	producer.wg.Add(goroutinesCount)
...
func (p *asyncProducer) ProduceSync(value sarama.Encoder, options ...MsgOption) <-chan *AsyncResult {
	message := messageWithOptions(value, p.topic, options...)
